...
const FlashcardReview: React.FC<FlashcardReviewProps> = ({ cards, onComplete, userId, appLanguage }) => {
  ...
  const handleRating = (rating: 'again' | 'hard' | 'good' | 'easy') => {
    ...
    updateFlashcard(updatedCard, userId);
  ...
  if (sessionComplete) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] animate-fade-in">
        ...
        <h2 className="text-3xl font-bold mb-2 text-slate-800 dark:text-white">{t.reviewComplete}</h2>
        <p className="text-slate-500 mb-8">{t.noDueCards}</p>
        <Button onClick={onComplete} className="px-8">{t.backDashboard}</Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-xl mx-auto py-10 px-4">
      <div className="flex justify-between items-center mb-6">
        <Button variant="outline" size="sm" onClick={onComplete}>{t.exit}</Button>
        <span className="text-sm font-bold text-brand-600">{queue.length} {t.cardShort} Left</span>
      </div>

      <div className="perspective-1000 h-96 relative mb-8">
        <div 
        ...
        >
          ...
          <div className="absolute w-full h-full backface-hidden bg-white dark:bg-slate-800 rounded-3xl shadow-xl border-2 border-slate-100 dark:border-slate-700 flex flex-col items-center justify-center p-8 text-center hover:border-brand-300 transition-colors">
            ...
            <h3 className="text-2xl md:text-3xl font-bold leading-relaxed">{currentCard.front}</h3>
