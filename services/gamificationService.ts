...
const saveUserInternal = (user: UserProfile) => {
  try {
    const users = JSON.parse(localStorage.getItem('studygenius_users') || '{}');
    users[user.email] = user;
    localStorage.setItem('studygenius_users', JSON.stringify(users));
    localStorage.setItem('studygenius_active_user', user.email);
  } catch (e) {
...
export const checkBadges = (user: UserProfile): { user: UserProfile, newBadges: string[] } => {
  ...
  BADGES.forEach(badge => {
    if (!currentBadges.has(badge.id)) {
      if (badge.condition(user, historyCount)) {
